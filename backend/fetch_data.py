...
def fetch_and_save(fyers, symbol, timeframe_name, timeframe_value):
    try:
        ...
        folder = os.path.join(DATA_FOLDER, timeframe_name)
...
def fetch_all():
    print("Starting data fetch for all F&O stocks...")
    print(f"Data will be saved to: {DATA_FOLDER}")
    print(f"Symbols: {len(FNO_SYMBOLS)}")
    ...
    total = len(FNO_SYMBOLS) * len(TIMEFRAMES)
    done = 0
    success = 0
    failed = 0
    ...
    print(f"Data saved at: {DATA_FOLDER}")
